...
async function insert(tableName, object) {
  ...
  const keysToString = Object.keys(cleanedObject)
    .map((key) => JSON.stringify(key))
    .join(',')
  const valueIndexes = Object.keys(cleanedObject)
    .map((_, index) => `$${index + 1}`)
    .join(',')
...
async function handleDirectRelations(contentType: string, item: any) {
  ...
  contentType = contentType.replaceAll('-', '_')
  const relations = Object.keys(schema.attributes).filter((attr: string) => schema.attributes[attr].type === 'relation')
  await Promise.all(
      ...
      if (defaultLinks) {
        await Promise.all(
          defaultLinks?.map(async (link) => {
            ...
            link[linkTargetId] =
              relationGI.find((item) => item.default.id === link[linkTargetId])?.localizations[0]?.id || undefined
...
async function handleComponentsRelations(contentType: string, item: any) {
  ...
  await Promise.all(
    componentWithRelations.map(async (component) => {
      ...
      await Promise.all(
        ...
        `)) as unknown as any[]
          ...
          const relations = Object.keys(component.schema.attributes).filter(
            (attr: string) => component.schema.attributes[attr].type === 'relation'
          )
