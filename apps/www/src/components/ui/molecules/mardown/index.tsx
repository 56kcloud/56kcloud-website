        ...
        <Markdown
          options={{
            createElement(type, props: React.HTMLProps<HTMLDivElement>, children: React.ReactNode) {
              ...
              if (element === 'video') {
                return <iframe key={props['href']} src={props['href']} className='w-full rounded-lg h-[500px]' />
              } else if (element?.type === 'img') {
                return (
                  <div className='flex flex-col items-center justify-center' key={element.props['src']}>
                    <DynamicImage src={element.props['src']} className='!mb-0 bg-white' />
                  </div>
                )
                ...
                return isFromTwitter(element.props['href']) ? (
                  ...
                  <Bookmark url={element.props['href']} key={element.props['href']} />
                )
              } else {
